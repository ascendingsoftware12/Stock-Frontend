...
function Procurement() {
  ...
  useEffect(() => {
    if (!pFlag) fetchProcurment();
    if (!sFlag) fetchStoreBrandModel();
  }, []);
  ...
  function downloadExcel(array) {
    ...
    link.download = "REPORT_PROCUREMENT.xlsx";
    link.click();
  }
  ...
  const handleExcel = async () => {
    ...
    try {
      const response = await axios.get(
        `/procurement/notapprovedexcel?projecteddays=${Projectiondays}`,
  ...
  const handleSearch = async () => {
    setLoading(true);
    // console.log(Ftstorecode)
    // console.log(Fmodelnumber);
    // console.log(Fbrand);
    try {
      const response = await axios.get(
        `procurement/notapproved?state=${state}&to_store_code=${ToStoreCode}&model_number=${modelNumber}&itemname=${itemName}&brand=${brand}&projecteddays=${Projectiondays}`
      );
      // const ProcurementData = response.data;
      // setProcurement(ProcurementData);
      let ProcurementData = response.data;
  ...
  const GetProjection = async () => {
    setLoading(true);
    try {
      const response = await axios.get("/procurement/notapproved", {
        params: {
          // affordablequantity: Affordablequantity,
          projecteddays: Projectiondays,
          state: state,
  ...
  const handleSaveClick = async () => {
    try {
      const selectedValues = selectedItems.map((item) => ({
        ...
        projection_days: parseInt(item.projection_days, 10) || 0,
        // affordable_quantity: parseFloat(Affordablequantity) || 0,
      }));

      const response = await axios.post("/procurement/save", selectedValues);
